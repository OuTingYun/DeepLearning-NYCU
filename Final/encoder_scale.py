...
class Encoder(nn.Module):
    ...
    def __init__(self, z_dim):
        ...
        self.block_size = [0,3,12]
        self.encoder_residual_blocks_group = nn.ModuleList([
            EncoderResidualBlock(z_dim // 8),
            EncoderResidualBlock(z_dim // 8),
            EncoderResidualBlock(z_dim // 8),
    ...
    def forward(self, x):
        ...
        for e, r in zip(self.encoder_blocks, self.encoder_residual_blocks):
            ...
            if g_idx!=2:
                for enc in self.encoder_residual_blocks_group[self.block_size[g_idx]:self.block_size[g_idx+1]]:
                    x = enc(x)
                    xs.append(x)
