...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Trainer for state AutoEncoder model.")
    parser.add_argument("--epochs", type=int, default=1000,
                        help="number of epochs.")
    parser.add_argument("--batch_size", type=int, default=128,
                        help="size of each sample batch")
    parser.add_argument("--dataset_path", type=str,
                        default="n01820546", help="dataset path")
    ...
    train_ds = ImageFolderDataset(dataset_path, mode='train', img_dim=64)
    train_dataloader = DataLoader(
        train_ds, batch_size=batch_size, shuffle=True, num_workers=opt.n_cpu)

    val_ds = ImageFolderDataset(dataset_path, mode='val', img_dim=64)
    val_dataloader = DataLoader(
        val_ds, batch_size=batch_size, shuffle=True, num_workers=opt.n_cpu)
    ...
    model = NVAE(z_dim=512, img_dim=(64, 64))   # 512-> 最後一層的 channel 數量
    ...
    if opt.pretrained_weights:
        model.load_state_dict(torch.load(
            opt.pretrained_weights, map_location=device), strict=False)

    warmup_kl = WarmupKLLoss(init_weights=[
                                1., 
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i, image in enumerate(val_dataloader):
                ...
                log_str = "\r---- [Epoch %d/%d, Step %d/%d] loss: %.6f----" % (
                    epoch, epochs, i, len(val_dataloader), loss.item())
                logging.info(log_str)
            ...
            if best_loss > val_loss:
                ...
                best_model = f"checkpoints/ae_ckpt_%d_%.6f.pth" % (
                    epoch, val_loss)
                torch.save(model.state_dict(),
                           best_model)

    best_model = f"checkpoints/ae_ckpt_%d_%.6f_final.pth" % (epoch, val_loss)
