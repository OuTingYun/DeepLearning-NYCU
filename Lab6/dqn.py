...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        '''epsilon-greedy based on behavior network'''
         ## TODO ##
        if random.random() < epsilon:   #探索 explore
            return action_space.sample()    # 使用隨機的動作(可能為新的)  
        else:   #exploit 用舊的
            with torch.no_grad():   # 迴船從 behavior net 中預測出最高分的動作的 位置 ！！
    ...
    def _update_behavior_network(self, gamma):
        ...
        ## TODO ##
        q_value = self._behavior_net(state).gather(dim=1,index=action.long())   #取之前選的位子

        with torch.no_grad():
           q_next =  self._target_net(next_state).max(dim=1)[0].view(-1,1)
           q_target = reward + gamma*q_next*(1-done)    # if done=False the (1-done)=1 else (1-done)=0
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                print(f'Step:{total_steps:10} | Episode: {episode:4} | Length: {t:4} | Total reward: {total_reward:7.2f} | Ewma reward: {ewma_reward:7.2f} | Epsilon: {epsilon:5.3f}')
                break
    env.close()
...
def test(args, env, agent, writer):
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        state = env.reset()
        ## TODO ##
        for t in itertools.count(start=1):  # 瘋狂執行
            env.render()
            # select action
            action = agent.select_action(state, epsilon, action_space)
            # execute action
...
def main():
    ...
    # parser.add_argument('--episode', default=2, type=int)
    ...
    writer = SummaryWriter('runs/'+args.logdir)
    agent = DQN(args)
